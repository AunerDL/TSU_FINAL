...
public class ListasSimples {
    ...
    private NodoSimples PrimerNodo;
    private NodoSimples UltimoNodo;
    ...
    public ListasSimples(String nombre) {
        this.nombre = nombre;
    ...
    public NodoSimples getPrimerNodo() {
        return this.PrimerNodo;
    }

    public NodoSimples getUltimoNodo() {
    ...
    public NodoSimples obtenerNodoEnPosicion(int posicion) {
        if (posicion < 0) {
    ...
    public void insertarEnfrente(Object elemento) {
        if (estaVacia()) {
            this.PrimerNodo = this.UltimoNodo = new NodoSimples(elemento, null, null);
        } else {
            this.PrimerNodo = new NodoSimples(elemento, PrimerNodo, null);
    ...
    public void insertarAlFinal(Object elemento) {
        if (estaVacia()) {
            PrimerNodo = UltimoNodo = new NodoSimples(elemento, null, null);
        } else {
            UltimoNodo.setSigNodo(new NodoSimples(elemento, null, null));
    ...
    public void insertarEnPosicion(Object elemento, int posicion) {
        ...
        NodoSimples nuevoNodo = new NodoSimples(elemento, null, null);
        NodoSimples actual = PrimerNodo;
    ...
    public NodoSimples busquedaSecuencial(Object elemento) {
        NodoSimples actual = PrimerNodo;
        while (actual != null) {
    ...
    public NodoSimples busquedaBinaria(Object elemento) {
        ordenamientoMerge();
        ...
        while (inicio <= fin) {
            ...
            NodoSimples nodoMedio = obtenerNodoEnPosicion(medio);
    ...
    public void ordenamientoBurbuja() {
        ...
        do {
            intercambio = false;
            NodoSimples actual = PrimerNodo;
            NodoSimples siguiente = PrimerNodo.getSigNodo();
    ...
    public void ordenamientoMerge() {
        ...
        NodoSimples nodoOrdenado = mergeSortRecursivo(PrimerNodo, tamaño);
        PrimerNodo = nodoOrdenado;
    }

    private NodoSimples mergeSortRecursivo(NodoSimples inicio, int tamaño) {
        if (tamaño <= 1) {
            return inicio;
        }

        int mitad = tamaño / 2;
        NodoSimples mitadNodo = inicio;
        for (int i = 0; i < mitad - 1; i++) {
            mitadNodo = mitadNodo.getSigNodo();
        }

        NodoSimples inicioSegundaMitad = mitadNodo.getSigNodo();
        mitadNodo.setSigNodo(null);

        NodoSimples primerMitad = mergeSortRecursivo(inicio, mitad);
        NodoSimples segundaMitad = mergeSortRecursivo(inicioSegundaMitad, tamaño - mitad);

        return merge(primerMitad, segundaMitad);
    }

    private NodoSimples merge(NodoSimples primerMitad, NodoSimples segundaMitad) {
        NodoSimples resultado = null;
    ...
    private NodoSimples obtenerUltimoNodo() {
        NodoSimples actual = PrimerNodo;
